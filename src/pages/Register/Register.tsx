...
export default function Register() {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    registerAccountMutation.mutate(body, {
      ...
      onError: (error) => {
        if (isAxiosBadRequest<ErrorResponseApi<Omit<FormData, 'confirm_password'>>>(error)) {
  ...
  return (
    <div className='w-full max-h-[900px] max-w-[400px] rounded-lg border border-gray-200 bg-white shadow-md p-10'>
      ...
      <form onSubmit={onSubmit} noValidate>
        ...
        <Button
          type='submit'
          className='w-full bg-primary py-2 text-sm text-white font-bold rounded-md hover:opacity-[0.8] flex items-center justify-center'
      ...
      <div className='font-semibold text-sm mt-5'>
        ...
        <Link className='text-sm text-primary hover:opacity-[0.8]' to={path.login}>
          Login in
        </Link>
      </div>
    </div>
